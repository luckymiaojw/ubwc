...
`endif
    ...
    generate
        ...
        for (g_lane = 0; g_lane < NUM_LANES; g_lane = g_lane + 1) begin : gen_lane
            ...
            wire                                            op_we_a_lane                   ;
            wire                                            op_we_b_lane                   ;
            wire        [ADDR_W              -1 :0]         op_addr_a_lane                 ;
            wire        [ADDR_W              -1 :0]         op_addr_b_lane                 ;
            wire        [RD_DATA_W           -1 :0]         op_din_a_lane                  ;
            wire        [RD_DATA_W           -1 :0]         op_din_b_lane                  ;
            wire        [RD_DATA_W           -1 :0]         bank_a_dout_lane               ;
            wire        [RD_DATA_W           -1 :0]         bank_b_dout_lane               ;
            ...
            assign op_cs_a_lane      = op_cs_a[g_lane];
            assign op_cs_b_lane      = op_cs_b[g_lane];
            assign op_we_a_lane      = op_we_a[g_lane];
            assign op_we_b_lane      = op_we_b[g_lane];
            assign op_addr_a_lane    = op_addr_a[g_lane];
            assign op_addr_b_lane    = op_addr_b[g_lane];
            assign op_din_a_lane     = op_din_a[g_lane];
            assign op_din_b_lane     = op_din_b[g_lane];
            assign bank_a_dout[g_lane] = bank_a_dout_lane;
            assign bank_b_dout[g_lane] = bank_b_dout_lane;
