...
module ubwc_enc_otf_data_packer
    ...
    assign otf_ready                  = rst_n_otf && format_supported && ~in_fifo_full;
    ...
    assign din_fcnt                   = otf_fcnt;
    assign din_lcnt                   = otf_lcnt;
    assign in_fifo_din                = {otf_last_beat, din_fcnt, din_lcnt, din_vsync, din_hsync, otf_data};
    assign inf_last_beat              = in_fifo_dout[146];
    assign inf_fcnt                   = in_fifo_dout[145:142];
    assign inf_lcnt                   = in_fifo_dout[141:130];
    assign inf_vsync_raw              = in_fifo_dout[129];
    assign inf_hsync_raw              = in_fifo_dout[128];
    assign inf_vsync                  = inf_vsync_raw | frame_bind_pending;
    assign inf_hsync                  = inf_hsync_raw | frame_bind_pending;
    assign inf_sideband               = {inf_fcnt, inf_lcnt, inf_vsync, inf_hsync};
    assign frame_bind                 = in_fifo_rd && frame_bind_pending;
    assign inf_data                   = in_fifo_dout[127:0];
    assign line_has_uv                = !inf_lcnt[0];
    ...
    assign in_fifo_rd                 = rst_n_sys && stream_enable &&
                                        !in_fifo_empty && !pipe_stall;
    ...
    always @(posedge i_clk or negedge rst_n_sys) begin
        if (!rst_n_sys)
            frame_bind_pending <= 1'b0;
        else if (i_frame_start)
            frame_bind_pending <= 1'b1;
        else if (frame_bind)
            frame_bind_pending <= 1'b0;
    end
