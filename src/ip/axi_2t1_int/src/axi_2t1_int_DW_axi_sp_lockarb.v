...
module axi_2t1_int_DW_axi_sp_lockarb (
  // Inputs - System.
  aclk_i,
  aresetn_i,
...
//----------------------------------------------------------------------
  ...
  `define axi_2t1_int_IMPLEMENT_LOCKING ((LOCKING==1) && (NC>1))
...
//----------------------------------------------------------------------
  ...
  // lock_other_i, gated off when locking not implemented.
  assign lock_other_msk = `axi_2t1_int_IMPLEMENT_LOCKING ? lock_other_i : 1'b0;
  ...
  generate
   if (`axi_2t1_int_IMPLEMENT_LOCKING)
   begin
    if (AWCH)
    begin
     always @(*) 
  ...
  assign locktx = `axi_2t1_int_IMPLEMENT_LOCKING 
                  ? ((rlocktx & rreq_i) 
                     | (wlocktx & wreq_i)
                    )
                  : {NC{1'b0}};
  ...
  // this channel. 
  assign outstnd_txs = 
    (`axi_2t1_int_IMPLEMENT_LOCKING==1)
  ...
  assign outstnd_txs_fed_o = (`axi_2t1_int_IMPLEMENT_LOCKING==1)
                             ? (outstnd_txs_r && (!outstnd_txs))
                             : 1'b0;
  ...
  assign outstnd_txs_nonlkd = (`axi_2t1_int_IMPLEMENT_LOCKING==1) 
                              ? (outstnd_txs_nonlkd_o
                                  || outstnd_txs_nonlkd_i
                                )
                              : 1'b0;
  ...
  assign unlocking_tx_rcvd_r_jstfd = (`axi_2t1_int_IMPLEMENT_LOCKING==1)
                                     ? unlocking_tx_rcvd_r
                                     : 1'b0;


  // Unlocking transaction received on either address channel.
  assign unlocking_tx_rcvd = `axi_2t1_int_IMPLEMENT_LOCKING
  ...
  //spyglass enable_block FlopEConst
  //spyglass enable_block STARC05-2.2.3.3
  assign this_ch_locked = `axi_2t1_int_IMPLEMENT_LOCKING ? this_ch_locked_r : 1'b0;
  ...
  assign both_ch_locked = `axi_2t1_int_IMPLEMENT_LOCKING ? both_ch_locked_r : 1'b0;
