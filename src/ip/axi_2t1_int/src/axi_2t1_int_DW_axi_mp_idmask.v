...
module axi_2t1_int_DW_axi_mp_idmask (
  // Inputs - System.
  aclk_i,
  aresetn_i,
...
//----------------------------------------------------------------------
  ...
  input [`axi_2t1_int_ID_W-1:0]           id_i; // Incoming ID value aligned with
  ...
  input [`axi_2t1_int_ID_W-1:0]     cpl_id_i; // ID of the transaction that
 ...
 // output [LOG2_MAX_UIDA*MAX_UIDA*MAX_CA_ID-1:0] act_ids_buffer; // buffer for active ids on address channel
  ...
  // Net not used in Read adddress channel intantiation.
  reg [`axi_2t1_int_ID_W*MAX_UIDA-1:0] act_ids_r;  // Regster containing all active
                                       // ID values.
  ...
  reg [`axi_2t1_int_ID_W*MAX_UIDA-1:0] act_ids_nxt; // Pre register versions of
  ...
  wire [`axi_2t1_int_AXI_MAX_NUM_MST_SLVS-1:0] shrd_here_has_shrd_ddctd_bus;
  ...
  // link.
  wire [`axi_2t1_int_AXI_MAX_NUM_MST_SLVS-1:0] shrd_ddctd_s_bus;
  ...
  always @(*) begin : shrd_here_has_shrd_ddctd_bus_vis_PROC
    ...
    for(ss=0;ss<`axi_2t1_int_AXI_MAX_NUM_USR_MSTS;ss=ss+1) begin
      if(vis_bus[ss]) begin
  ...
  begin : tx_in_shrd_ddctd_pipe_mask_r_PROC
    if(~aresetn_i) begin
      ...
      if(  local_slv_mux_thru_shrd_ddctd
         ...
         & (`axi_2t1_int_AXI_AW_PL_ARB | `axi_2t1_int_AXI_AW_SHARED_PL)
        ) begin
  ...
  begin : act_ids_snums_nxt_PROC
    ...
    begin
     ...
     if(tx_issued && freeslot_oh[slot_num]) begin
       ...
       for(id_bit=0 ; id_bit<=(`axi_2t1_int_ID_W-1) ; id_bit=id_bit+1) begin
          act_ids_nxt[(slot_num*`axi_2t1_int_ID_W)+id_bit]
          = id_mux[id_bit];
       end
  ...
  begin : act_ids_snums_r_PROC
    if(!aresetn_i) begin
      ...
      act_ids_r   <=   {`axi_2t1_int_ID_W*MAX_UIDA{1'b0}};
  ...
  begin : cpl_slot_num_oh_PROC
    ...
    begin
      ...
      // bit by bit.
      for(id_bit=0 ; id_bit<=(`axi_2t1_int_ID_W-1) ; id_bit=id_bit+1) begin
        id[id_bit] = act_ids_r[(slot_num*`axi_2t1_int_ID_W)+id_bit];
      end
  ...
  assign act_ids_o = `axi_2t1_int_REG_INTER_BLOCK_PATHS
                     ? act_ids_r
                     : act_ids_nxt;

  assign act_snums_o  = `axi_2t1_int_REG_INTER_BLOCK_PATHS
                        ? act_snums_r
                        : act_snums_nxt;
...
assign tx_issued_valid = valid_i & ready_mst_i ; // when a tx is accepted
  ...
  begin: act_ids_buffer_PROC 
    ...
    if(tx_issued_valid || (act_ids_rd_buffer_pointer == act_ids_buffer_pointer)) begin
       for(id_bit=0 ; id_bit<=(`axi_2t1_int_ID_W-1) ; id_bit=id_bit+1) begin
         act_ids_buffer[(act_ids_buffer_pointer*`axi_2t1_int_ID_W)+id_bit]
          = id_i[id_bit]; 
      end
    end   
  ...
  generate 
    if (`axi_2t1_int_AXI_REG_AW_W_PATHS)
