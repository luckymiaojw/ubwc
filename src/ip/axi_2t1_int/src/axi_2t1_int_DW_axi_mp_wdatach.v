...
module axi_2t1_int_DW_axi_mp_wdatach (
  // Inputs - System.
  aclk_i,
  aresetn_i,
...
//----------------------------------------------------------------------
  ...
  parameter PYLD_M_W = `axi_2t1_int_AXI_W_PYLD_M_W; // W channel payload width
                                        // from master.
  parameter PYLD_S_W = `axi_2t1_int_AXI_W_PYLD_S_W; // W channel payload width to
                                        // slave.
...
//----------------------------------------------------------------------
  ...
  `define axi_2t1_int_S0_VIS  ( S0_N_VIS ||  S0_B_VIS)
  `define axi_2t1_int_S1_VIS  ( S1_N_VIS ||  S1_B_VIS)
  `define axi_2t1_int_S2_VIS  ( S2_N_VIS ||  S2_B_VIS)
  `define axi_2t1_int_S3_VIS  ( S3_N_VIS ||  S3_B_VIS)
  `define axi_2t1_int_S4_VIS  ( S4_N_VIS ||  S4_B_VIS)
  `define axi_2t1_int_S5_VIS  ( S5_N_VIS ||  S5_B_VIS)
  `define axi_2t1_int_S6_VIS  ( S6_N_VIS ||  S6_B_VIS)
  `define axi_2t1_int_S7_VIS  ( S7_N_VIS ||  S7_B_VIS)
  `define axi_2t1_int_S8_VIS  ( S8_N_VIS ||  S8_B_VIS)
  `define axi_2t1_int_S9_VIS  ( S9_N_VIS ||  S9_B_VIS)
  `define axi_2t1_int_S10_VIS (S10_N_VIS || S10_B_VIS)
  `define axi_2t1_int_S11_VIS (S11_N_VIS || S11_B_VIS)
  `define axi_2t1_int_S12_VIS (S12_N_VIS || S12_B_VIS)
  `define axi_2t1_int_S13_VIS (S13_N_VIS || S13_B_VIS)
  `define axi_2t1_int_S14_VIS (S14_N_VIS || S14_B_VIS)
  `define axi_2t1_int_S15_VIS (S15_N_VIS || S15_B_VIS)
  `define axi_2t1_int_S16_VIS (S16_N_VIS || S16_B_VIS)

  `define axi_2t1_int_PAYLOAD_W (PYLD_M_W + ICM_PORT*(`axi_2t1_int_AXI_LOG2_NM))
  `define axi_2t1_int_ID_W (`axi_2t1_int_AXI_MIDW + ICM_PORT*(`axi_2t1_int_AXI_LOG2_NM))
 
  // If Pipe Line is used then the buffer width of act_ids_buffer is
  // increased by 1 
  localparam PL_BUF = (`axi_2t1_int_AXI_AW_TMO == 0)?0:1;
...
//----------------------------------------------------------------------
  ...
  input                   valid_i; // Valid from external
                                   // master.
  input [`axi_2t1_int_PAYLOAD_W-1:0]  payload_i; // Payload from external
                                     // master.
  // Outputs - External Master.
  ...
  input [`axi_2t1_int_ID_W-1:0] id_rs_i;
  ...
  wire [`axi_2t1_int_AXI_NSP1-1:0] sys_slv_oh; // One hot system slave number.
  // Max sized version of sys_slv_oh.
  wire [`axi_2t1_int_AXI_MAX_NUM_MST_SLVS-1:0] sys_slv_oh_max;

  wire valid_mskd; // Masked version of valid_i from the id mask
                   // block.

  wire [`axi_2t1_int_AXI_LOG2_NM-1:0] ms_num; // Wire to hold the master system
  ...
  wire [`axi_2t1_int_ID_W-1:0] id; // wid extracted from payload.
  ...
  wire [`axi_2t1_int_ID_W-1:0] id_mux;
  ...
  // registered depending on `axi_2t1_int_AXI_REG_AW_W_PATHS.
  wire [ACT_IDS_W-1:0]   act_ids_mux;
  wire [ACT_SNUMS_W-1:0] act_snums_mux;
  wire [MAX_UIDA-1:0] issuedtx_slot_oh_mux;
  reg [`axi_2t1_int_PAYLOAD_W-1:0]  payload_i_wire; // Payload from external master
  wire valid_lp;
  ...
  assign act_ids_mux = `axi_2t1_int_AXI_REG_AW_W_PATHS ? act_ids_r : act_ids_i;
  assign act_snums_mux = `axi_2t1_int_AXI_REG_AW_W_PATHS ? act_snums_r : act_snums_i;


  
    reg  [LOG2_ACT_ID_BUF_POINTER_WIDTH-1:0]    act_ids_buffer_pointer;
    reg [`axi_2t1_int_ID_W-1:0] id_dummy;       // awid extracted from act_id_buffer.
   ...
   begin : payload_id_assign_PROC
    integer pyld_bit;
        for(pyld_bit=0 ; pyld_bit<=(`axi_2t1_int_PAYLOAD_W-1) ; pyld_bit=pyld_bit+1) begin
            if ( pyld_bit <`axi_2t1_int_AXI_WPYLD_ID_RHS_M+`axi_2t1_int_ID_W  && pyld_bit >= `axi_2t1_int_AXI_WPYLD_ID_RHS_M)
                payload_i_wire[pyld_bit] = id_dummy[pyld_bit-`axi_2t1_int_AXI_WPYLD_ID_RHS_M] ;
  ...
  assign last = payload_i_wire[`axi_2t1_int_AXI_WPYLD_LAST];
  ...
  // Are AW to W paths registered.
  assign issuedtx_slot_oh_mux = `axi_2t1_int_AXI_REG_AW_W_PATHS
                                ? issuedtx_slot_oh_r
                                : issuedtx_slot_oh_i;
  ...
  begin : id_slot_match_oh_PROC
    ...
    reg [`axi_2t1_int_ID_W-1:0]         slot_id;
    ...
    for(slotnum=0; slotnum<=(MAX_UIDA-1) ; slotnum=slotnum+1) begin
      ...
      for(idbit=0; idbit<=(`axi_2t1_int_ID_W-1) ; idbit=idbit+1) begin
        slot_id[idbit] = act_ids_mux[(slotnum*`axi_2t1_int_ID_W)+idbit];
      end
  ...
  axi_2t1_int_DW_axi_busmux_ohsel
  ...
  axi_2t1_int_DW_axi_lcltosys
  
  #(NUM_VIS_SP,       // Number of local slaves.
    LOG2_NUM_VIS_SP,
    `axi_2t1_int_AXI_NSP1,        // Number of slaves on this interconnect instance.
    `axi_2t1_int_AXI_LOG2_NSP1,

    // Port visibility parameters.
     `axi_2t1_int_S0_VIS,  `axi_2t1_int_S1_VIS,  `axi_2t1_int_S2_VIS,  `axi_2t1_int_S3_VIS,  `axi_2t1_int_S4_VIS,  `axi_2t1_int_S5_VIS,
     `axi_2t1_int_S6_VIS,  `axi_2t1_int_S7_VIS,  `axi_2t1_int_S8_VIS,  `axi_2t1_int_S9_VIS, `axi_2t1_int_S10_VIS, `axi_2t1_int_S11_VIS,
    `axi_2t1_int_S12_VIS, `axi_2t1_int_S13_VIS, `axi_2t1_int_S14_VIS, `axi_2t1_int_S15_VIS, `axi_2t1_int_S16_VIS,
  ...
  assign sys_slv_oh_max = {
                           {(`axi_2t1_int_AXI_MAX_NUM_MST_SLVS -`axi_2t1_int_AXI_NSP1){1'b0}},
                           sys_slv_oh};
  ...
  generate
   if (`axi_2t1_int_AXI_NUM_SYS_MASTERS == 1 || ICM_PORT == 1)
   begin
    always @(*)
   ...
   begin
    ...
    begin : append_mstnum_to_id_PROC
      integer pyld_bit;
      integer id_bit;
      reg [`axi_2t1_int_AXI_SIDW-1:0] sidw;

      id_bit = 0;
      sidw = {ms_num,
              payload_i_wire[`axi_2t1_int_AXI_WPYLD_ID_LHS_M:`axi_2t1_int_AXI_WPYLD_ID_RHS_M]};
      for(pyld_bit=0 ; pyld_bit<=(PYLD_S_W-1) ; pyld_bit=pyld_bit+1) begin
        // Assign fields up to id field.
        if(pyld_bit<`axi_2t1_int_AXI_WPYLD_ID_RHS_M) begin
          payload_o[pyld_bit] = payload_i_wire[pyld_bit];

        // Assign id field.
        end else if(   (pyld_bit>=`axi_2t1_int_AXI_WPYLD_ID_RHS_M)
                     && (pyld_bit<=`axi_2t1_int_AXI_WPYLD_ID_LHS_M+`axi_2t1_int_AXI_LOG2_NM)
                     )
        begin
        ...
        // Assign fields after id fields.
          ...
          payload_o[pyld_bit] = payload_i_wire[pyld_bit-`axi_2t1_int_AXI_LOG2_NM];
        end
        // spyglass enable_block SelfDeterminedExpr-ML      
      end
    end // append_mstnum_to_id_PROC
  ...
  begin : act_ids_buffer_pointer_wch_PROC
    if(!aresetn_i) 
      ...
      if(last && valid_lp && ready_i) begin                                      //if any transcation is getting over (wlast assetrion)
          if(act_ids_buffer_pointer == ((ACT_ID_BUF_POINTER_WIDTH/`axi_2t1_int_ID_W)-1)  )            // when pointer reaches to maximum buffer size
  ...
  begin: act_ids_buffer_slotid_read_PROC 
    integer id_bit ;
    id_dummy = {(`axi_2t1_int_ID_W){1'b0}};
      for(id_bit=0 ; id_bit<=(`axi_2t1_int_ID_W-1) ; id_bit=id_bit+1) begin                     // recover the  id value from  act_ids_buffer and store it to id_dummy
        id_dummy[id_bit] = act_ids_buffer[(act_ids_buffer_pointer*`axi_2t1_int_ID_W)+id_bit];  // "act_ids_buffer_pointer" points to the  slot  number to be read     
     end  
  ...
  `undef axi_2t1_int_PAYLOAD_W
  `undef axi_2t1_int_ID_W
