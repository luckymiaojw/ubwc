...
module axi_2t1_int_DW_axi_sp_wrorder (
  // Inputs - System.
  aclk_i,
  aresetn_i,
...
//----------------------------------------------------------------------
  ...
  // interleaving slots.
  `define axi_2t1_int_BUS_SLOT_MST_W (LOG2_NUM_VIS_MP*WID)

  // Number of input busses to each mst require mux.
  `define axi_2t1_int_MST_REQ_MUX_NUM_DIN 2

  // Width of master require bus mux data in bus.
  `define axi_2t1_int_MST_REQ_BMUX_DIN_W (`axi_2t1_int_MST_REQ_MUX_NUM_DIN*LOG2_NUM_VIS_MP)

  // Width of master require multi bus mux data in bus.
  `define axi_2t1_int_MST_REQ_MBMUX_DIN_W (`axi_2t1_int_MST_REQ_BMUX_DIN_W*WID)
...
//----------------------------------------------------------------------
  ...
  //--------------------------------------------------------------------
  // This signal is unused if there is only single master port connected to this slave port 
  reg [`axi_2t1_int_BUS_SLOT_MST_W-1:0] bus_slot_mst_r; // Local master numbers for
  ...
  reg [`axi_2t1_int_MST_REQ_MBMUX_DIN_W-1:0] mst_req_mux_din; // Data in busses for
                                                  // the master require
                                                        // mux.
  ...
  assign can_send_vld_frm_aw =
    (   issued_tx_i
      ...
      & ((WID==1) | (`axi_2t1_int_AXI_AW_PL_ARB==1))
      & ((HAS_W_SHRD_DDCTD_LNK==0) | (`axi_2t1_int_AXI_AW_SHARED_PL==1))
      & (`axi_2t1_int_AXI_REG_AW_W_PATHS==0)
      & (MCA_EN==0)
    );
               ...
               & (  (!fifo_empty)
                  | (~(   ((WID==1) | (`axi_2t1_int_AXI_AW_PL_ARB==1))
                       & (  (HAS_W_SHRD_DDCTD_LNK==0)
                          | (`axi_2t1_int_AXI_AW_SHARED_PL==1)
                         )
                       & (MCA_EN==0)
                       & (`axi_2t1_int_AXI_REG_AW_W_PATHS==0)
                     ))
...
//spyglass enabale_block W528
  ...
  // result in very long logic paths.  `axi_2t1_int_AXI_REG_AW_W_PATHS macro is
  ...
  begin : bus_slot_mst_r_PROC
    if(!aresetn_i) begin
      bus_slot_mst_r <= {`axi_2t1_int_BUS_SLOT_MST_W{1'b0}};
    end else begin
      for(slotnum=0 ; slotnum<=(WID-1) ; slotnum=slotnum+1) begin
  ...
  begin : mst_req_mux_din_PROC
    ...
    mst_req_mux_din = {`axi_2t1_int_MST_REQ_MBMUX_DIN_W{1'b0}};
    ...
    for(slotnum=0 ; slotnum<=(WID-1) ; slotnum=slotnum+1) begin
      ...
      for(busnum=0 ;
          busnum<=(`axi_2t1_int_MST_REQ_MUX_NUM_DIN-1) ;
           busnum=busnum+1
          )
      begin
          ...
          for(mstbit=0 ;
           ...
           begin
            mst_req_mux_din[(  LOG2_NUM_VIS_MP
                               *(slotnum*`axi_2t1_int_MST_REQ_MUX_NUM_DIN)
                            )
  ...
  // the channel arbiter.
  axi_2t1_int_DW_axi_multibusmux
  
  #(`axi_2t1_int_MST_REQ_MUX_NUM_DIN, // Number of inputs to the mux.
  ...
  #(NUM_VIS_MP,           // Number of inputs to the mux.
    `axi_2t1_int_AXI_MST_PRIORITY_W,  // Width of each input to the mux.
    LOG2_NUM_VIS_MP,      // Width of select line for the mux.
    WID                   // Number of busmuxes to implement.
  )
