...
`define axi_2t1_int_REG_VID             4'h0
`define axi_2t1_int_REG_HWC             4'h4
`define axi_2t1_int_REG_CMD             4'h8
`define axi_2t1_int_REG_DATA            4'hC
`define axi_2t1_int_COMMAND_EN_BIT      31
`define axi_2t1_int_WR_BIT              30
`define axi_2t1_int_SR_BIT              29
`define axi_2t1_int_MID_BIT_MSB         11
`define axi_2t1_int_CH_ID_BIT           7
`define axi_2t1_int_QOS_REG_OFFSET      3 

module axi_2t1_int_DW_axi_apb_regfile (
     pclk,
     presetn,
     wr_en,
     rd_en,
   ...
   //------------------------------------------------------
     ...
     input  [`axi_2t1_int_IC_ADDR_SLICE_LHS-2:0]    reg_addr;       //APB register offset
     input  [`axi_2t1_int_MAX_APB_DATA_WIDTH-1:0]   ipwdata;        //Write data from APB BIU 
     output [`axi_2t1_int_MAX_APB_DATA_WIDTH-1:0]   iprdata;        //Internal read data bus
  ...
  //Local Parameters
  localparam [31:0] VID_REG         = `axi_2t1_int_DW_AXI_VERSION_ID; //  version id of corekit , driven by coreConsultant param
  localparam        APB3BIT         = `axi_2t1_int_AXI_HAS_APB3;
  localparam        QOSBIT          = `axi_2t1_int_AXI_HAS_QOS;
  localparam        LOCKENBIT       = `axi_2t1_int_AXI_HAS_LOCKING;
  localparam        TZENBIT         = `axi_2t1_int_AXI_HAS_TZ_SUPPORT;
  localparam        EXTDECBIT       = `axi_2t1_int_AXI_HAS_XDCDR;
  localparam        REMAPENBIT      = `axi_2t1_int_AXI_REMAP_EN;
  localparam        BICMDENBIT      = `axi_2t1_int_AXI_HAS_BICMD;
  localparam  [0:0] AXI_HAS_AXI4    = (`axi_2t1_int_AXI_INTERFACE_TYPE == 0) ? 0 : 1;
  localparam        LPENBIT         = `axi_2t1_int_AXI_LOWPWR_HS_IF; 
  localparam  [4:0] MST_NUM         = `axi_2t1_int_AXI_NUM_MASTERS;
  localparam  [4:0] SLV_NUM         = `axi_2t1_int_AXI_NUM_SLAVES;
  ...
  // ------------------------------------------------------
   ...
   //  SW-registers 
   // ------------------------------------------------------
   assign apb_reg_vid_en       = (reg_addr == (`axi_2t1_int_REG_VID >> 2));
   assign apb_reg_hwconfig_en  = (reg_addr == (`axi_2t1_int_REG_HWC >> 2));
   assign apb_reg_cmd_en       = (reg_addr == (`axi_2t1_int_REG_CMD >> 2));
   assign apb_reg_data_en      = (reg_addr == (`axi_2t1_int_REG_DATA >> 2));
   ...
   always @(posedge pclk or negedge presetn) begin : APBWR_CMDREG_WRITE_PROC
        if(presetn == 1'b0) 
            ...
            {cmd_reg[31],cmd_reg[29]}              <= {(cmd_reg[31] & ((~command_en_ack) | (cmd_reg[`axi_2t1_int_SR_BIT] & (~internal_rst)))), 
                                                       (cmd_reg[31] & (~command_en_ack) & cmd_reg[`axi_2t1_int_SR_BIT])}; //Auto clearing of cmd_reg bits  
  ...
  always @(posedge pclk or negedge presetn) begin : APBWR_DATAREG_WRITE_PROC
    ...
    always @(*)       begin : APB_REG_READ_PROC
     ...
     assign command_en       =   cmd_reg[`axi_2t1_int_COMMAND_EN_BIT]; 
     ...
     assign internal_rst =   command_en_ack & cmd_reg[`axi_2t1_int_SR_BIT];
     assign internal_reg_rst =   cmd_reg[`axi_2t1_int_SR_BIT];
     ...
     assign qosreg_wr_en      =   cmd_reg[`axi_2t1_int_WR_BIT]  &  command_en ; 
            // write command for internal qos register
     
     assign qosreg_rd_en      =   (~cmd_reg[`axi_2t1_int_WR_BIT])  & command_en ;
            // read command for internal qos register
     
     assign qosreg_offset   =   {cmd_reg[`axi_2t1_int_MID_BIT_MSB],cmd_reg[`axi_2t1_int_MID_BIT_MSB-1],cmd_reg[`axi_2t1_int_MID_BIT_MSB-2] ,cmd_reg[`axi_2t1_int_MID_BIT_MSB-3], cmd_reg[`axi_2t1_int_CH_ID_BIT] ,cmd_reg[`axi_2t1_int_QOS_REG_OFFSET-1:0] }; 
