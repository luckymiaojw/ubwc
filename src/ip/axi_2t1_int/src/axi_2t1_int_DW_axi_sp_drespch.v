...
module axi_2t1_int_DW_axi_sp_drespch (
  // Inputs - System.
  //aclk_i,
  //aresetn_i,
...
//----------------------------------------------------------------------
  ...
  parameter PYLD_M_W = `axi_2t1_int_AXI_R_PYLD_M_W; // Payload width to master.
  parameter PYLD_S_W = `axi_2t1_int_AXI_R_PYLD_S_W; // Payload width from slave.
...
// LOCAL MACROS.
//----------------------------------------------------------------------
  `define axi_2t1_int_DRESPCH_ID_RHS ((R_CH==1) ? `axi_2t1_int_AXI_RPYLD_ID_RHS_S : `axi_2t1_int_AXI_BPYLD_ID_RHS_S)
  `define axi_2t1_int_DRESPCH_ID_LHS ((R_CH==1) ? `axi_2t1_int_AXI_RPYLD_ID_LHS_S : `axi_2t1_int_AXI_BPYLD_ID_LHS_S)
...
//----------------------------------------------------------------------
  ...
  // REGISTER VARIABLES.
  //--------------------------------------------------------------------
  reg [`axi_2t1_int_AXI_LOG2_NM-1:0] id_mstnum; // Master number taken from 
                                    // slaves ID signal.
  ...
  wire [`axi_2t1_int_AXI_MAX_NUM_USR_MSTS-1:0] shared_m_bus;
  wire [`axi_2t1_int_AXI_SIDW-1:0] id_slv; // ID signal from slave.

  // One hot system master number from systolcl block.
  wire [`axi_2t1_int_AXI_MAX_NUM_USR_MSTS-1:0] bidi_sys_pnum_oh;
  reg [`axi_2t1_int_AXI_MAX_NUM_USR_MSTS-1:0] id_mstnum_oh;
  ...
  assign id_slv = payload_i[`axi_2t1_int_DRESPCH_ID_LHS:`axi_2t1_int_DRESPCH_ID_RHS];
  ...
  begin : C_ID_MSTNUM_PROC
   ...
   // 1 master in the system.
     for(mnum_bit=0 ; 
         mnum_bit<=(`axi_2t1_int_AXI_LOG2_NM-1) ; 
         mnum_bit=mnum_bit+1
        ) 
     begin
       id_mstnum[mnum_bit] = id_slv[mnum_bit+`axi_2t1_int_AXI_MIDW];
     end
  end // id_mstnum_PRO
  ...
  // slave port.
  wire [`axi_2t1_int_AXI_MAX_NUM_USR_MSTS-1:0] vis_m_bus;
  assign vis_m_bus
    = {(M15_VIS ? 1'b1 : 1'b0),
       (M14_VIS ? 1'b1 : 1'b0),
       (M13_VIS ? 1'b1 : 1'b0),
  ...
  always @(*) begin : id_mstnum_oh_PROC
    integer mst;
    id_mstnum_oh = {`axi_2t1_int_AXI_MAX_NUM_USR_MSTS{1'b0}};
    for(mst=0 ; mst<`axi_2t1_int_AXI_MAX_NUM_USR_MSTS ; mst=mst+1) begin
  ...
  assign mst_on_shrd = HAS_SHARED 
                       ? |(  shared_m_bus 
                           & (`axi_2t1_int_AXI_HAS_BICMD ? bidi_sys_pnum_oh 
                                             : id_mstnum_oh
                             )
                          )
                         : 1'b0 ;
  ...
  wire [`axi_2t1_int_AXI_MAX_NUM_USR_MSTS-1:0] shared_m_a_bus;
  ...
  #(
    ...
    `axi_2t1_int_AXI_NUM_MASTERS,  // Number of slaves in system, including default
                       // slave.
    `axi_2t1_int_AXI_LOG2_NM,      // Log base 2 of number of slaves in the system.
  ...
  begin : strip_mstnum_from_id_PROC
    integer pyld_bit;
    integer id_bit;
    reg [`axi_2t1_int_AXI_SIDW-1:0] midw;

    id_bit = 0;
    midw = payload_i[`axi_2t1_int_DRESPCH_ID_LHS:`axi_2t1_int_DRESPCH_ID_RHS];

    for(pyld_bit=0 ; pyld_bit<=(PYLD_M_W-1) ; pyld_bit=pyld_bit+1) begin
      // Assign fields up to id field.
      if(pyld_bit<`axi_2t1_int_DRESPCH_ID_RHS) begin
        payload_o[pyld_bit] = payload_i[pyld_bit];

      // Assign id field.
      end else if(   (pyld_bit>=`axi_2t1_int_DRESPCH_ID_RHS) 
                  && (pyld_bit<=(`axi_2t1_int_DRESPCH_ID_LHS-`axi_2t1_int_AXI_LOG2_NM))
                 ) 
      begin
      ...
      // Assign fields after id fields.
        ...
        // SMD: Self determined expression found
        // SJ : The expression indexing the vector/array will never exceed the bound of the vector/array.        
          payload_o[pyld_bit] = payload_i[pyld_bit+`axi_2t1_int_AXI_LOG2_NM];
        // spyglass enable_block SelfDeterminedExpr-ML   
      end
    end
  ...
  assign cpl_tx_o = valid_i & ready_i && 
                    (payload_i[`axi_2t1_int_AXI_RPYLD_LAST] || (!R_CH));
