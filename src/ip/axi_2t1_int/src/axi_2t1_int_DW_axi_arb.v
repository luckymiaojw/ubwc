...
//----------------------------------------------------------------------
  ...
  localparam MASTERS_PRIORITY_W =(ARB_TYPE==4)? `axi_2t1_int_AXI_QOSW: PRIORITY_W;

//----------------------------------------------------------------------
// LOCAL MACROS.
//----------------------------------------------------------------------
  `define axi_2t1_int_ARB_PARK_INDEX    0 // Macro for park index parameter of
                              // instantiated designware arbiter.
            
  `define axi_2t1_int_ARB_OP_MODE_NOREG 0 // Macro for output mode parameter of
  ...
  `define axi_2t1_int_NC_INTERNAL  ((NC==1) ? 2 : NC)            

  // Macro for priority bus to the arbiter, using NC_INTERNAL macro.
  `define axi_2t1_int_BUS_PRIOR_W_INTERNAL (`axi_2t1_int_NC_INTERNAL*MASTERS_PRIORITY_W)
...
// Outputs - Channel Destination.
  ...
  wire [`axi_2t1_int_NC_INTERNAL-1:0] bus_grant_arb_o;   //
  ...
  // for these signals.
  wire [`axi_2t1_int_NC_INTERNAL-1:0]          req_arb;
  wire [`axi_2t1_int_BUS_PRIOR_W_INTERNAL-1:0] bus_priorities_inv_arb;

  wire arb_enable; // Signal to disable/stall the arbiters.


  // Multi cycle arbitration signals.
  wire [`axi_2t1_int_NC_INTERNAL-1:0] req_mca; // Request signals from multi cycle
  ...
  wire [`axi_2t1_int_BUS_PRIOR_W_INTERNAL-1:0] bus_priorities_mca;
  ...
  wire parked_unconn;
    // Pass 2 bits if NC == 1.
  wire [`axi_2t1_int_NC_INTERNAL-1:0] use_other_pri_int; 
  //wire [`axi_2t1_int_NC_INTERNAL-1:0] bus_grant_lock_int;
  //Not used if NC==1 
  wire [(`axi_2t1_int_NC_INTERNAL*ARB_INDEX_W)-1:0] bus_priority_other_int;
  wire [(`axi_2t1_int_NC_INTERNAL*ARB_INDEX_W)-1:0] bus_priority_o_int;
  //--------------------------------------------------------------------
  ...
  generate 
    ...
    begin
     ...
     assign bus_priority_other_int = {(`axi_2t1_int_NC_INTERNAL*ARB_INDEX_W){1'b0}};
    end
  endgenerate   
 ...
 //VP:: To avoid lint error assigned unused bits to 0
  generate
    if (`axi_2t1_int_BUS_PRIOR_W_INTERNAL == BUS_MASTERS_PRIORITY_W) 
      assign bus_priorities_inv_arb = bus_priorities_inv;
    else
      assign bus_priorities_inv_arb = {{(`axi_2t1_int_BUS_PRIOR_W_INTERNAL -BUS_MASTERS_PRIORITY_W){1'b0}}, bus_priorities_inv};
  endgenerate  
  ...
  */
  ...
  //--------------------------------------------------------------------
  axi_2t1_int_DW_axi_mca_reqhold
  
  #(MCA_EN,                // 1 if multi cycle arbitration is enabled.
    MCA_HLD_PRIOR,         // 1 if priority bits should be registered.
    `axi_2t1_int_BUS_PRIOR_W_INTERNAL, // Width if priority signal bus.
    `axi_2t1_int_NC_INTERNAL,          // Number of request signals.
  ...
  generate 
    case (ARB_TYPE)
      ...
      `axi_2t1_int_AXI_ARB_TYPE_DP : begin : gen_arb_type_dp
        
      assign bus_priority_o_int = {`axi_2t1_int_NC_INTERNAL*ARB_INDEX_W{1'b0}};
        axi_2t1_int_DW_axi_arbiter_dp
        
        #(`axi_2t1_int_NC_INTERNAL,       // Number of clients.
          MASTERS_PRIORITY_W,         // Priority signal width per client.   
          ARB_PARK_MODE,     
          `axi_2t1_int_ARB_PARK_INDEX,    
          `axi_2t1_int_ARB_OP_MODE_NOREG,
        ...
        U_DW_axi_arbiter_dp (
          ...
          .request      (req_mca),
          .lock         ({`axi_2t1_int_NC_INTERNAL{1'b0}}), // Not using lock inputs.
          .mask         ({`axi_2t1_int_NC_INTERNAL{1'b0}}), // Not using mask inputs.
      ...
      `axi_2t1_int_AXI_ARB_TYPE_FCFS : begin : gen_arb_type_fcfs
        axi_2t1_int_DW_axi_arbiter_fcfs
      ...
      `axi_2t1_int_AXI_ARB_TYPE_2T : begin : gen_arb_type_2t
        axi_2t1_int_DW_axi_arbiter_fae
 ...
 `axi_2t1_int_AXI_ARB_TYPE_QOS : begin : gen_arb_type_qos
