...
module axi_2t1_int_DW_axi_mp_drespch (
  // Inputs - System.  
  aclk_i,
  aresetn_i,
...
//----------------------------------------------------------------------
  ...
  parameter BUS_PYLD_S_W = (4*`axi_2t1_int_AXI_R_PYLD_M_W);  // Width of bus with 
                                                 // payloads from all 
                                                 // visible slaves.

  parameter PYLD_M_W = `axi_2t1_int_AXI_R_PYLD_M_W;  // Width of payload bus to 
                                         // master.

  parameter PYLD_S_W = `axi_2t1_int_AXI_R_PYLD_M_W;  // Width of individual
...
//----------------------------------------------------------------------
  ...
  //`define axi_2t1_int_PER_SLAVE_ARBITRATION (MAX_UIDA>=NUM_VIS_SP)
  ...
  `define axi_2t1_int_PER_SLAVE_ARBITRATION 1

  // Macros for arbiter parameters depending on whether we are 
  // arbitrating for all visible slaves or for the number of unique 
  // ID values.
  `define axi_2t1_int_ARB_NC (`axi_2t1_int_PER_SLAVE_ARBITRATION ? NUM_VIS_SP : MAX_UIDA)
  `define axi_2t1_int_ARB_PRIOR_W     (`axi_2t1_int_PER_SLAVE_ARBITRATION ? BUS_PRIORITY_W : (MAX_UIDA*`axi_2t1_int_AXI_LOG2_NSP1))

  `define axi_2t1_int_ARB_LOG2_NC (`axi_2t1_int_PER_SLAVE_ARBITRATION ? LOG2_NUM_VIS_SP : LOG2_MAX_UIDA)

  // Note this value only works for per slave arbitration.
  // Since we never use per UIDA arbitration at the moment.
  `define axi_2t1_int_ARB_LOG2_NC_P1 LOG2_NUM_VIS_SP_P1

  `define axi_2t1_int_BUS_PORT_REQ_W (`axi_2t1_int_PER_SLAVE_ARBITRATION ? (NUM_VIS_SP*LOG2_NUM_VIS_SP) : ACT_SNUMS_W)

  // Switch ID field macros depending on which channel is being
  // implemented.
  `define axi_2t1_int_DRESPCH_ID_RHS ((R_CH==1) ? `axi_2t1_int_AXI_RPYLD_ID_RHS_M : `axi_2t1_int_AXI_BPYLD_ID_RHS_M)
  `define axi_2t1_int_DRESPCH_ID_LHS ((R_CH==1) ? `axi_2t1_int_AXI_RPYLD_ID_LHS_M : `axi_2t1_int_AXI_BPYLD_ID_LHS_M)
  ...
  `define axi_2t1_int_PYLD_ICM_W (PYLD_M_W + ICM_PORT*(`axi_2t1_int_AXI_LOG2_NM))
...
//----------------------------------------------------------------------
  ...
  output [`axi_2t1_int_PYLD_ICM_W-1:0]  payload_icm_o; 
  ...
  output [`axi_2t1_int_ID_W-1:0]      cpl_id_o; // completion signals. 
  ...
  wire [`axi_2t1_int_AXI_MAX_NUM_MST_SLVS-2:0] has_ddctd_m_bus_max;
  // Above sized for NSM.
  ...
  DW_axi_multibusmux
  #(NUM_VIS_SP,           // Number of inputs to the mux.
    `axi_2t1_int_AXI_SLV_PRIORITY_W,  // Width of each input to the mux.
    LOG2_NUM_VIS_SP,      // Width of select line for the mux.
    MAX_UIDA              // Number of busmuxes to implement.
  )
  ...
  */
  ...
  #(ARB_TYPE,                 // Type of arbitration used.
    ...
    `axi_2t1_int_AXI_MCA_HLD_PRIOR,       // Hold priorities for multi cycle arb.
    `axi_2t1_int_AXI_SLV_PRIORITY_W,      // Priority width of a single slave.                 
    `axi_2t1_int_ARB_PRIOR_W,             // Width of priorities bus.
  ...
  U_DW_axi_arb (
    ...
    .last_i               (payload_pre_irs[`axi_2t1_int_AXI_RPYLD_LAST]),
  ...
  assign cpl_tx_o 
   = R_CH 
     ? payload_o[`axi_2t1_int_AXI_RPYLD_LAST] & valid_o & ready_i
     : valid_o & ready_i;
  ...
  assign cpl_tx_shrd_bus_o 
    = {NSM{ ((payload_o[`axi_2t1_int_AXI_RPYLD_LAST] | (R_CH==0)) & ready_shrd_mux)  }}
      & bus_valid_shrd_o;                    
  ...
  generate
    if(SHARED) begin : gen_mp_dresp_shared
      assign cpl_id_o 
        = payload_o[`axi_2t1_int_DRESPCH_ID_RHS+`axi_2t1_int_ID_W-1:`axi_2t1_int_DRESPCH_ID_RHS];
