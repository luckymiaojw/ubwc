...
module axi_2t1_int_DW_axi_lockdec (
  // Inputs.
  bus_awpayload_i,
  bus_arpayload_i,
...
//----------------------------------------------------------------------
  ...
  // REGISTER VARIABLES.
  //--------------------------------------------------------------------
  reg [`axi_2t1_int_AXI_AW_PYLD_S_W-1:0] awpayload; // Temporary variables for a 
  reg [`axi_2t1_int_AXI_AR_PYLD_S_W-1:0] arpayload; // single payload.
  ...
  begin : bus_awlocktx_o_PROC
    ...
    for(mp=0 ; mp<=(NUM_VIS_MP-1) ; mp=mp+1) begin
      ...
      for(bt=0 ; bt<=(`axi_2t1_int_AXI_AW_PYLD_S_W-1) ; bt=bt+1) begin
  awpayload[bt] = bus_awpayload_i[(mp*`axi_2t1_int_AXI_AW_PYLD_S_W)+bt];
      end

      bus_awlocktx_o[mp] 
        = (     awpayload[`axi_2t1_int_AXI_AWPYLD_LOCK_LHS:`axi_2t1_int_AXI_AWPYLD_LOCK_RHS]
       == `axi_2t1_int_AXI_LT_LOCK
    );
  ...
  begin : bus_arlocktx_o_PROC
    ...
    for(mp=0 ; mp<=(NUM_VIS_MP-1) ; mp=mp+1) begin
      ...
      for(bt=0 ; bt<=(`axi_2t1_int_AXI_AR_PYLD_S_W-1) ; bt=bt+1) begin
  arpayload[bt] = bus_arpayload_i[(mp*`axi_2t1_int_AXI_AR_PYLD_S_W)+bt];
      end

      bus_arlocktx_o[mp] 
        = (     arpayload[`axi_2t1_int_AXI_ARPYLD_LOCK_LHS:`axi_2t1_int_AXI_ARPYLD_LOCK_RHS]
       == `axi_2t1_int_AXI_LT_LOCK
    );
