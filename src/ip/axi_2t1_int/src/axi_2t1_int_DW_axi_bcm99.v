...
module axi_2t1_int_DW_axi_bcm99 (
  clk_d,
  rst_d_n,
  data_s,
  data_d
...
// { START Latency Accurate modeling
  initial begin : set_setup_hold_delay_PROC
      `define axi_2t1_int_DW_HOLD_MUX_DELAY  1
      if (ACCURATE_MISSAMPLING == 1)
        $display("Information: %m: *** Warning: `axi_2t1_int_DW_HOLD_MUX_DELAY is not defined so it is being set to: %0d ***", `axi_2t1_int_DW_HOLD_MUX_DELAY);

      `define axi_2t1_int_DW_SETUP_MUX_DELAY  1
      if (ACCURATE_MISSAMPLING == 1)
        $display("Information: %m: *** Warning: `axi_2t1_int_DW_SETUP_MUX_DELAY is not defined so it is being set to: %0d ***", `axi_2t1_int_DW_SETUP_MUX_DELAY);
  ...
  #`axi_2t1_int_DW_HOLD_MUX_DELAY clk_d_q = 1'b1;

  always @ (negedge clk_d)
  #`axi_2t1_int_DW_HOLD_MUX_DELAY clk_d_q = 1'b0;
  
  //Delay the source data
  assign #`axi_2t1_int_DW_SETUP_MUX_DELAY data_s_q = (!rst_d_n) ? {WIDTH{1'b0}}:data_s;
