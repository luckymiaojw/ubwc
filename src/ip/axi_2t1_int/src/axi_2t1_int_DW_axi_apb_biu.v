...
module axi_2t1_int_DW_axi_apb_biu
(
 // APB bus interface
 pclk,
 presetn,
 ...
 iprdata
   ...
   input     [`axi_2t1_int_APB_ADDR_WIDTH-1:0]  paddr;     // APB address
   input                            pwrite;    // APB write/read
   input                            penable;   // APB enable
   input      [`axi_2t1_int_APB_DATA_WIDTH-1:0] pwdata;    // APB write data bus
   

   // -------------------------------------
   // -- Register block interface signals
   // -------------------------------------
   input  [`axi_2t1_int_MAX_APB_DATA_WIDTH-1:0] iprdata;   // Internal read data bus


   output     [`axi_2t1_int_APB_DATA_WIDTH-1:0] prdata;    // APB read data bus

   output                           wr_en;     // Write enable signal
   output                           rd_en;     // Read enable signal
   output                     [3:0] byte_en;   // Active byte lane signal
   output  [`axi_2t1_int_IC_ADDR_SLICE_LHS-2:0] reg_addr;  // Register address offset
   output [`axi_2t1_int_MAX_APB_DATA_WIDTH-1:0] ipwdata;   // Internal write data bus

   // -------------------------------------
   // -- Local registers & wires
   // -------------------------------------
   reg        [`axi_2t1_int_APB_DATA_WIDTH-1:0] prdata;    // Registered prdata output
   reg    [`axi_2t1_int_MAX_APB_DATA_WIDTH-1:0] ipwdata;   // Internal pwdata bus
  //Only required amount of bits used. Rest will be optimized
   ...
   // -- APB address bus
   // --------------------------------------------
   assign reg_addr = paddr[`axi_2t1_int_IC_ADDR_SLICE_LHS:2];
  ...
  //SJ: This is not an issue.
   always @(pwdata) begin : IPWDATA_PROC
      ipwdata = { `axi_2t1_int_MAX_APB_DATA_WIDTH{1'b0} };
      ipwdata[`axi_2t1_int_APB_DATA_WIDTH-1:0] = pwdata[`axi_2t1_int_APB_DATA_WIDTH-1:0];
   end
  //spyglass enable_block W415a
   ...
   always @(posedge pclk or negedge presetn) begin : PRDATA_PROC
      if(presetn == 1'b0) begin
         prdata <= { `axi_2t1_int_APB_DATA_WIDTH{1'b0} };
      end else begin
         if(rd_en) begin
                  prdata <= iprdata;
         end
