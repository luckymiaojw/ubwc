...
module axi_2t1_int_DW_axi_sp_wdatach (
  // Inputs - System.
  aclk_i,
  aresetn_i,
...
//----------------------------------------------------------------------
  ...
  parameter BUS_PYLD_M_W = (2*`axi_2t1_int_AXI_W_PYLD_M_W); 

  parameter PYLD_S_W = `axi_2t1_int_AXI_W_PYLD_M_W; // Payload width to slave. 
...
//----------------------------------------------------------------------
  ...
  `define axi_2t1_int_ARB_NC ((NUM_CON_MP==1) ? 1 : (SHARED ? NUM_CON_MP : WID))

  // Log base 2 of number of clients to the arbiter.
  `define axi_2t1_int_ARB_LOG2_NC ((NUM_CON_MP==1) ? 1 : (SHARED ? LOG2_NUM_CON_MP : LOG2_WID))
  
  // Log base 2 of (number of clients to the arbiter + 1).
  `define axi_2t1_int_ARB_LOG2_NC_P1 ((NUM_CON_MP==1) ? 1 : (SHARED ? LOG2_NUM_CON_MP_P1 : LOG2_WID_P1))

  // Width of priorities bus from the wrorder block, priorities for
  // 0 to WID-1.
  `define axi_2t1_int_BUS_PRIORITY_WID_W (WID*`axi_2t1_int_AXI_MST_PRIORITY_W)

  // Width of bus containing master numbers of masters which are
  // being allowed to access the arbiter.
  `define axi_2t1_int_BUS_PORT_REQ_W (WID*LOG2_NUM_CON_MP)
  ...
  `define axi_2t1_int_USE_INT_GRANT_INDEX 0

  `define axi_2t1_int_REG_INTER_BLOCK_PATHS 0

  // Width of arbiter internal grant index.                                
  localparam ARB_INDEX_W = (ARB_TYPE==1) 
                           ? `axi_2t1_int_ARB_LOG2_NC_P1 
                           : `axi_2t1_int_ARB_LOG2_NC;
...
//----------------------------------------------------------------------
  ...
  wire [`axi_2t1_int_BUS_PRIORITY_WID_W-1:0] bus_priorities_wrorder;            

  // Bus containing master numbers who are allowed to request to the
  // arbiter.
  wire [`axi_2t1_int_BUS_PORT_REQ_W-1:0] bus_mst_req_wrorder;
  reg [`axi_2t1_int_BUS_PORT_REQ_W-1:0] bus_mst_req_wrorder_r; // Reg'd version.
  wire [`axi_2t1_int_BUS_PORT_REQ_W-1:0] bus_mst_req_pla_mux; // Selected from above
                                                  // 2 depending on 
                                                  // pipelining.
  ...
  wire [`axi_2t1_int_ARB_NC-1:0] req_arb; // Selected from bus_valid_i[0]
                                       // or req_wrorder depending on
                                       // number of visible master
                                       // ports.

  reg [`axi_2t1_int_ARB_NC-1:0] req_arb_r; // registered version of 
                                        // req_arb.
           
  wire [`axi_2t1_int_ARB_NC-1:0] req_arb_mux; // Selected from req_arb
                                           // and req_arb_r depending
                                           // on PL_ARB.

  // Selected from 0 or bus_priorities_wrorder, depending on number of
  // visible master ports.
  wire [`axi_2t1_int_BUS_PRIORITY_WID_W-1:0] bus_priorities_arb;            

  // Selected from 0 or bus_mst_req_wrorder, depending on number of
  // visible master ports.
  wire [`axi_2t1_int_BUS_PORT_REQ_W-1:0] bus_mst_req_arb;

  wire grant; // Grant output from DW_axi_arb.

  // Arbiter grant index.
  wire [`axi_2t1_int_ARB_LOG2_NC-1:0] arb_grant_index; 
  // Arbiter 1 hot grant output.
  wire [`axi_2t1_int_ARB_NC-1:0] arb_bus_grant; 
  ...
  wire [`axi_2t1_int_AXI_MAX_NUM_USR_MSTS-1:0] aw_shrd_param_bus;
  wire [`axi_2t1_int_AXI_MAX_NUM_USR_MSTS-1:0] w_shrd_param_bus;
  ...
  wire [`axi_2t1_int_AXI_MAX_NUM_USR_MSTS-1:0] vis_bus;
  ...
  wire [`axi_2t1_int_AXI_MAX_NUM_MST_SLVS-1:0] has_ddctd_s_bus_max;
  // Above sized for NSS.
  ...
  wire [(`axi_2t1_int_ARB_NC*ARB_INDEX_W)-1:0] bus_priority_unconn;
  ...
  always @(*) begin : aw_shrd_param_bus_lcl_PROC
    ...
    for(sm=0;sm<`axi_2t1_int_AXI_MAX_NUM_USR_MSTS;sm=sm+1) begin
      if(vis_bus[sm]) begin
  ...
  */
  ...
  assign issued_tx_mux = `axi_2t1_int_REG_INTER_BLOCK_PATHS
                         ? issued_tx_r : issued_tx_c;

  assign issued_mstnum_mux = `axi_2t1_int_REG_INTER_BLOCK_PATHS
                             ? issued_mstnum_r : issued_mstnum_c;

  assign issued_tx_shrd_slv_oh_mux 
  = `axi_2t1_int_REG_INTER_BLOCK_PATHS
  ...
  generate 
    if(SHARED == 0) begin : gen_sp_wdatach_w_not_shared
      ...
      #(NUM_CON_MP,           // Number of visible master ports.
        ...
        BUS_PRIORITY_W,       // Width of bus with priorities per 
                              // visible master.
        `axi_2t1_int_BUS_PRIORITY_WID_W,  // Width of bus with priorities per write
                              // interleaving depth.
        `axi_2t1_int_BUS_PORT_REQ_W,      // Width of bus containing master numbers 
                              // which
     ...
     assign    bus_mst_req_wrorder = {`axi_2t1_int_BUS_PORT_REQ_W{1'b0}};
     assign    bus_priorities_wrorder = {`axi_2t1_int_BUS_PRIORITY_WID_W{1'b0}};
    ...
    if(SHARED) begin : gen_sp_wdatach_w_shared
      ...
      //----------------------------------------------------------------
      axi_2t1_int_DW_axi_sp_shrd_wrorder
  ...
  assign bus_mst_req_arb = (NUM_CON_MP==1) 
                           ?{`axi_2t1_int_BUS_PORT_REQ_W{1'b0}}
                           : bus_mst_req_wrorder;

  assign bus_priorities_arb = (NUM_CON_MP==1)
                              ?{`axi_2t1_int_BUS_PRIORITY_WID_W{1'b0}}
  ...
  // Channel Arbiter
  //--------------------------------------------------------------------
  axi_2t1_int_DW_axi_arb
   
  #(ARB_TYPE,                 // Arbitration type.
    `axi_2t1_int_ARB_NC,                  // Number of clients to arbiter.
    `axi_2t1_int_ARB_LOG2_NC,             // Log base 2 of number of clients.
    `axi_2t1_int_ARB_LOG2_NC_P1,          // Log base 2 of (number of clients + 1).
    ...
    `axi_2t1_int_AXI_MST_PRIORITY_W,      // Priority width of a single master.            
    `axi_2t1_int_BUS_PRIORITY_WID_W,      // Width of priorities bus.
  ...
  U_DW_axi_arb (
    ...
    .locktx_i             ({`axi_2t1_int_ARB_NC{1'b0}}), // Not required here.
    .unlock_i             (1'b0), // Not required here.
    .grant_masked_i       (1'b0), // Not required here.

    // Not required here, next 2 inputs required on address
    // channels only.
    .use_other_pri_i      ({`axi_2t1_int_ARB_NC{1'b0}}), 
    //.bus_grant_lock_i     ({`axi_2t1_int_ARB_NC{1'b0}}),
  ...
  generate
    if(SHARED==0) begin : gen_sp_wdatach_dcd_not_shared
      ...
      begin : bus_mst_req_wrorder_r_PROC
        if(~aresetn_i) begin
          bus_mst_req_wrorder_r <= {`axi_2t1_int_BUS_PORT_REQ_W{1'b0}};
        end else begin
      ...
      #(`axi_2t1_int_ARB_NC,      // Number of input busses.
        LOG2_NUM_CON_MP,       // Width of each input bus.
        `axi_2t1_int_ARB_LOG2_NC  // Width of select line.
      ) 
      U_port_req_mux (
        .sel  (arb_grant_index),
  ...
  #(`axi_2t1_int_ARB_NC,     // Number of inputs to the mux.
    1,                    // Width of each input to the mux.
    `axi_2t1_int_ARB_LOG2_NC // Width of select input for the mux.
  )
  U_DW_axi_busmux_mca (
    .sel  (arb_grant_index),
    .din  (req_arb_mux), 
  ...
  assign cpl_tx = payload_pre_irs[`axi_2t1_int_AXI_WPYLD_LAST] & valid_o & ready_i;

  // Shared Channel
  // Detect write data part completion for each slave attached to the
  // shared channel.
  assign cpl_tx_shrd_bus = 
    (  {NSS{payload_pre_irs[`axi_2t1_int_AXI_WPYLD_LAST]}}
  ...
  #(((SHARED & SHARED_PL) ? `axi_2t1_int_AXI_TMO_FRWD : 0), // Channel timing option
  ...
  */
  ...
  `undef axi_2t1_int_ARB_NC
  `undef axi_2t1_int_ARB_LOG2_NC
  `undef axi_2t1_int_ARB_LOG2_NC_P1
  `undef axi_2t1_int_REG_INTER_BLOCK_PATHS 
