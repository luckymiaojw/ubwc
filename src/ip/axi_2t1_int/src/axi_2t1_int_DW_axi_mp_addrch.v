...
module axi_2t1_int_DW_axi_mp_addrch (
  aclk_i,
  aresetn_i,
...
//----------------------------------------------------------------------
  ...
  parameter PYLD_M_W = `axi_2t1_int_AXI_AR_PYLD_M_W;
  // Channel payload width to slave.
  parameter PYLD_S_W = `axi_2t1_int_AXI_AR_PYLD_S_W;
...
//----------------------------------------------------------------------
  ...
  input [`axi_2t1_int_PAYLOAD_W-1:0]  payload_i;  // Channel payload vector.
  ...
  input [`axi_2t1_int_ID_W-1:0]      cpl_id_i; // completion signals.
  ...
  //output [`axi_2t1_int_AXI_REGIONW-1:0]   region_o;
  ...
  reg [`axi_2t1_int_ID_W-1:0]      cpl_id_r; // signals, for timing considerations.
  ...
  reg [`axi_2t1_int_AXI_MAX_NUM_MST_SLVS-1:0] xdcd_slvnum_oh;
  ...
  wire [`axi_2t1_int_ID_W-1:0]     id_mst;   // ID and address signals from master.
  wire [`axi_2t1_int_AXI_AW-1:0]   addr_mst;

  // Wires for unconnected sub module ports.
  wire [`axi_2t1_int_AXI_LOG2_NSP1-1:0] sys_slv_unconn;
  wire [`axi_2t1_int_AXI_MAX_NUM_USR_MSTS-1:0] bidi_sys_pnum_oh_unconn;
  ...
  wire valid_lp;
  wire [`axi_2t1_int_AXI_REGIONW-1:0]   region_o;
 ...
 // wire [LOG2_MAX_UIDA*MAX_UIDA*MAX_CA_ID-1:0] act_ids_buffer; // buffer for active ids on address channel
  ...
  assign addr_mst
    = payload_i[`axi_2t1_int_AXI_ARPYLD_ADDR_LHS:`axi_2t1_int_AXI_ARPYLD_ADDR_RHS];

  assign id_mst
    = payload_i[`axi_2t1_int_AXI_AWPYLD_ID_RHS_M+`axi_2t1_int_ID_W-1:`axi_2t1_int_AXI_AWPYLD_ID_RHS_M];


  wire [ `axi_2t1_int_PAYLOAD_W-1:0]  payload_i_q;  // Channel payload vector.
  assign payload_i_q = payload_i;
  //--------------------------------------------------------------------
  // Instantiate the address decoder block.
  //--------------------------------------------------------------------
  axi_2t1_int_DW_axi_dcdr
  ...
  assign local_slv_mux = `axi_2t1_int_AXI_HAS_XDCDR
                         ? local_slv_xdcdr
                            : local_slv_idcdr;
 ...
 //   (`axi_2t1_int_AXI_HAS_TZ_SUPPORT & (local_slv_mux != local_slv_tzone))
 //   ? (SHARED_S0 == 1'b1)
 //   : `axi_2t1_int_AXI_HAS_XDCDR
 //     ? slv_on_shrd_xdcdr
 //     : slv_on_shrd_idcdr;
  ...
  assign slv_on_shrd_mux = (`axi_2t1_int_AXI_HAS_TZ_SUPPORT
    )
    ?
    (SHARED_S0 == 1'b1)
    : `axi_2t1_int_AXI_HAS_XDCDR
  ...
  //assign local_slv = `axi_2t1_int_AXI_HAS_TZ_SUPPORT
  //                   ? local_slv_tzone
  //                     : local_slv_mux;
  ...
  begin : completion_regs_PROC
    if(!aresetn_i) begin
      cpl_tx_r <= 1'b0;
      cpl_id_r <= {`axi_2t1_int_ID_W{1'b0}};
    end else begin
      cpl_tx_r <= cpl_tx_i;
      cpl_id_r <= cpl_id_i;
    end
  ...
  #(
    ...
    `axi_2t1_int_S0_VIS, `axi_2t1_int_S1_VIS, `axi_2t1_int_S2_VIS, `axi_2t1_int_S3_VIS, `axi_2t1_int_S4_VIS, `axi_2t1_int_S5_VIS, `axi_2t1_int_S6_VIS,
    `axi_2t1_int_S7_VIS, `axi_2t1_int_S8_VIS, `axi_2t1_int_S9_VIS, `axi_2t1_int_S10_VIS, `axi_2t1_int_S11_VIS, `axi_2t1_int_S12_VIS, `axi_2t1_int_S13_VIS,
    `axi_2t1_int_S14_VIS, `axi_2t1_int_S15_VIS, `axi_2t1_int_S16_VIS,
  ...
  generate 
   ...
   begin
    ...
    begin : append_mstnum_to_id_PROC
      ...
      sidw = {MS_NUM,
              payload_i_q[`axi_2t1_int_AXI_ARPYLD_ID_LHS_M:`axi_2t1_int_AXI_ARPYLD_ID_RHS_M]};
      ...
      for(pyld_bit=0 ;
       ...
       if(pyld_bit<`axi_2t1_int_AXI_ARPYLD_ID_RHS_M) begin
          payload_o_int[pyld_bit] = payload_i_q[pyld_bit];

        // Assign id field.
        end else if(   (pyld_bit>=`axi_2t1_int_AXI_ARPYLD_ID_RHS_M)
                     && (pyld_bit<=(`axi_2t1_int_AXI_ARPYLD_ID_LHS_M+`axi_2t1_int_AXI_LOG2_NM))
                     )
        begin
        ...
        // SJ : The expression indexing the vector/array will never exceed the bound of the vector/array. 
            payload_o_int[pyld_bit] = payload_i_q[pyld_bit-`axi_2t1_int_AXI_LOG2_NM];
  ...
  // blocks.
  `undef axi_2t1_int_S0_VIS
  `undef axi_2t1_int_S1_VIS
  `undef axi_2t1_int_S2_VIS
  `undef axi_2t1_int_S3_VIS
  `undef axi_2t1_int_S4_VIS
  `undef axi_2t1_int_S5_VIS
  `undef axi_2t1_int_S6_VIS
  `undef axi_2t1_int_S7_VIS
  `undef axi_2t1_int_S8_VIS
  `undef axi_2t1_int_S9_VIS
  `undef axi_2t1_int_S10_VIS
  `undef axi_2t1_int_S11_VIS
  `undef axi_2t1_int_S12_VIS
  `undef axi_2t1_int_S13_VIS
  `undef axi_2t1_int_S14_VIS
  `undef axi_2t1_int_S15_VIS
  `undef axi_2t1_int_S16_VIS
