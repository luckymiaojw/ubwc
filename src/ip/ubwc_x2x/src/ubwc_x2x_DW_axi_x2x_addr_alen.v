...
module ubwc_x2x_DW_axi_x2x_addr_alen (
  //inputs
  resize_ctrl_i,
  xact_upsize_i,
  mp_total_byte_i,
...
//  parameter MAX_SP_ALEN       = `ubwc_x2x_X2X_MAX_SP_ALEN+1;//{`ubwc_x2x_X2X_SP_BLW{1b1}}+1
  ...
  parameter MP_BLW            = `ubwc_x2x_X2X_MP_BLW;      //SP burst length width
  ...
  assign upper_bound_addr = {1'b0, bound_addr_i} + mp_total_byte_i;
  assign bound_plus_total = (resize_ctrl_i == `ubwc_x2x_MUL_XACT_OR_US_NOTLAST &&
                             (!xact_upsize_i) ) ? {1'b0, bound_addr_i} +
                             max_sp_mpasize_total_byte_i :
                             bound_addr_i + `ubwc_x2x_X2X_MAX_SP_TOTAL_BYTE;
  // spyglass disable_block W164a
  // SMD: LHS width is less than RHS width of assignment (Truncation)
  // SJ: Only lower bits of RHS is assigned to LHS. This is not a functional issue,
  // this is as per the requirement. Hence this can be waived.
  assign minus_aligned_addr = upper_bound_addr - {1'b0, addr_i};
  ...
  assign alen_s12 = (resize_ctrl_i == `ubwc_x2x_MUL_XACT_OR_US_INLAST &&
                     micro_ctrl_i == `ubwc_x2x_MXUI_DS) ?
                     (minus_aligned_addr >> asize_i) - 1 :
                     (minus_aligned_addr >> `ubwc_x2x_X2X_MAX_SP_ASIZE) - 1;
  ...
  //SMD: Reports cause of uncontrollability or unobservability and estimates the number of nets whose controllability / observability is impacted. 
  //SJ : Tool will issue unobservability warning only for those bits which are "not read" or "floating". Since we are not reading those bits we don't need observability. Hence waiving this warning.
  assign alen_s34 = (resize_ctrl_i == `ubwc_x2x_MUL_XACT_OR_US_NOTLAST &&
                    //VCS coverage off
                     (!xact_upsize_i) &&
                    //VCS coverage on
                     (micro_ctrl_i == `ubwc_x2x_MUX_OVERONE ||
                      micro_ctrl_i == `ubwc_x2x_MUX_CROSS)) ?
                     (minus_total_addr >> asize_i) - 1 :
                     (minus_total_addr >> `ubwc_x2x_X2X_MAX_SP_ASIZE) - 1;
  //spyglass enable_block TA_09
  ...
  assign addr_t_plus_sp_total = (resize_ctrl_i == 
                                 `ubwc_x2x_MUL_XACT_OR_US_NOTLAST) ?
                       (total_addr_r_i + max_sp_mpasize_total_byte_i) :
                       (total_addr_r_i + `ubwc_x2x_X2X_MAX_SP_TOTAL_BYTE);
  //spyglass enable_block W164a
  ...
  assign align_plus_sp_total = aligned_addr_i + `ubwc_x2x_X2X_MAX_SP_TOTAL_BYTE;
  assign alen_s  = (mp_total_byte_i >> `ubwc_x2x_X2X_MAX_SP_ASIZE) - 1;
  //spyglass disable_block TA_09
  //SMD: Reports cause of uncontrollability or unobservability and estimates the number of nets whose controllability / observability is impacted. 
  //SJ : Tool will issue unobservability warning only for those bits which are "not read" or "floating". Since we are not reading those bits we don't need observability. Hence waiving this warning.
  ...
  assign alen_s56 = (resize_ctrl_i == `ubwc_x2x_MUL_XACT_OR_US_NOTLAST &&
                     (!xact_upsize_i)) ?
                     ((mp_total_byte_i-
                      (total_alen_r_i << asize_i)) >> asize_i) - 1 :
                     ((mp_total_byte_i -
                      (total_alen_r_i << `ubwc_x2x_X2X_MAX_SP_ASIZE))
                      >> `ubwc_x2x_X2X_MAX_SP_ASIZE ) - 1;
  // spyglass enable_block SelfDeterminedExpr-ML
  // spyglass enable_block TA_09

  assign asize_mux = (resize_ctrl_i == `ubwc_x2x_MUL_XACT_OR_US_NOTLAST) ?
                      asize_i : `ubwc_x2x_X2X_MAX_SP_ASIZE;

  //if resize_ctrl_i == `ubwc_x2x_MUL_XACT_OR_US_INLAST
  ...
  parameter [SP_BLW-1:0]   MAX_SP_ALEN_DFLT       = {`ubwc_x2x_X2X_SP_BLW{1'b1}};
  parameter [LOG2_MP_BW:0] MAX_SP_TOTAL_ALEN_DFLT       = `ubwc_x2x_X2X_MAX_SP_ALEN+1;//{`ubwc_x2x_X2X_SP_BLW{1b1}}+1
  always @( micro_ctrl_i or alen_s or
    ...
    mxui_total_addr = total_addr_r_i;
      ...
      case ( micro_ctrl_i )
        ...
        `ubwc_x2x_MXUI_NO_CROSS, `ubwc_x2x_MXUI_FI_MUL, `ubwc_x2x_MXUI_FI_DS: begin
          mxui_total_alen = MAX_SP_TOTAL_ALEN_DFLT;
          mxui_alen = MAX_SP_ALEN_DFLT; // MAX_SP_ALEN - 1;
          if ( micro_ctrl_i == `ubwc_x2x_MXUI_NO_CROSS || 
          //VCS coverage off
               micro_ctrl_i == `ubwc_x2x_MXUI_FI_MUL ) 
          //VCS coverage on
...
//spyglass enable_block W415a
  ...
  //if resize_ctrl_i == `ubwc_x2x_MUL_XACT_OR_US_NOTLAST
  ...
  assign alen_plus_max = total_alen_r_i + (`ubwc_x2x_X2X_MAX_SP_ALEN+1);
  ...
  always @( micro_ctrl_i or 
    ...
    mxun_total_addr = total_addr_r_i;
      ...
      case ( micro_ctrl_i )
        `ubwc_x2x_MUX_OVERONE: begin
          if ( total_addr_r_i == upper_bound_addr ||
               (addr_t_plus_sp_total <= upper_bound_addr &&
                total_addr_r_i > {1'b0, addr_i}) ||
              (addr_t_plus_sp_total <= {1'b0, addr_i}) ) begin
...
//SJ : Here mxun_addr is assigned multiple times depend upon various conditions and expression as per the requirement.
        ...
        `ubwc_x2x_MUX_WRAP_BOUND, `ubwc_x2x_MUX_WRAP, 
        `ubwc_x2x_MUX_FI_LAST: begin
          mxun_alen = alen_s56;

          if (micro_ctrl_i == `ubwc_x2x_MUX_WRAP_BOUND) mxun_addr = bound_addr_i;

          // jstokes, 7.8.10, crm 9000406128, total_addr_r_i now
          // OPRT_AW+1 bits wide, mxun_addr needs only OPRT_AW bits.
          if ( micro_ctrl_i == `ubwc_x2x_MUX_WRAP ) 
            mxun_addr = total_addr_r_i[OPRT_AW-1:0];

          // jstokes, 7.8.10, crm 9000406128, total_addr_r_i now
          // OPRT_AW+1 bits wide, mxun_addr needs only OPRT_AW bits.
          if ( micro_ctrl_i == `ubwc_x2x_MUX_FI_LAST && aburst_i == `ubwc_x2x_X2X_BT_INCR )
            mxun_addr = total_addr_r_i[OPRT_AW-1:0]; //INCR
        end

        `ubwc_x2x_MUX_CROSS: begin
          // jstokes, 7.8.10, crm 9000406128, total_addr_r_i now
          // OPRT_AW+1 bits wide, mxun_addr needs only OPRT_AW bits.
          mxun_addr = total_addr_r_i[OPRT_AW-1:0];
        ...
        `ubwc_x2x_MUX_FI_MUL: begin
          ...
          if ( aburst_i == `ubwc_x2x_X2X_BT_INCR ) //INCR
            // jstokes, 7.8.10, crm 9000406128, total_addr_r_i now
            // OPRT_AW+1 bits wide, mxun_addr needs only OPRT_AW bits.
            mxun_addr = total_addr_r_i[OPRT_AW-1:0];
        end
//spyglass enable_block W415a
  ...
  //if resize_ctrl_i == `ubwc_x2x_MUL_XACT_INLAST
  ...
  always @( micro_ctrl_i or 
    ...
    mxi_reduce_num = reduce_num_r_i;
      ...
      case ( micro_ctrl_i )
        ...
        `ubwc_x2x_MXI_FI: begin
          if ( aburst_i == `ubwc_x2x_X2X_BT_FIXED )
            mxi_alen = MAX_SP_ALEN_DFLT; // MAX_SP_ALEN - 1;
          else begin //INCR
            if ( (`ubwc_x2x_X2X_MAX_SP_ALEN+1) > reduce_num_i ) begin
  // spyglass disable_block TA_09
  // SMD: Reports cause of uncontrollability or unobservability and estimates the number of nets whose controllability / observability is impacted.
  ...
  // spyglass enable_block TA_09
              ...
              mxi_alen = MAX_SP_ALEN_DFLT; // MAX_SP_ALEN - 1;
              mxi_stall = 1'b1;
              mxi_reduce_num = reduce_num_i - (`ubwc_x2x_X2X_MAX_SP_ALEN+1);//MAX_SP_ALEN;
            end
          end
  ...
  //if resize_ctrl_i == `ubwc_x2x_MUL_XACT_NOTLAST
  ...
  assign total_plus_sp_total = total_addr_r_i + `ubwc_x2x_X2X_MAX_SP_TOTAL_BYTE;//MAX_SP_TOTAL_BYTE;
//spyglass disable_block W415a 
//SMD: Signal may be multiply assigned (beside initialization) in the same scope.
//SJ : Here mxun_addr is assigned multiple times depend upon various conditions and expression as per the requirement.
  always @( micro_ctrl_i or 
    ...
    mxn_stall = 1'b0;
      ...
      case ( micro_ctrl_i )
        ...
        `ubwc_x2x_MUX_WRAP_BOUND, `ubwc_x2x_MUX_WRAP, 
        `ubwc_x2x_MUX_FI_LAST: begin
          if (micro_ctrl_i == `ubwc_x2x_MUX_FI_LAST && aburst_i == `ubwc_x2x_X2X_BT_FIXED)
  //spyglass disable_block TA_09
  //SMD: Reports cause of uncontrollability or unobservability and estimates the number of nets whose controllability / observability is impacted. 
  //SJ : Tool will issue unobservability warning only for those bits which are "not read" or "floating". Since we are not reading those bits we don't need observability. Hence waiving this warning.
  //spyglass disable_block SelfDeterminedExpr-ML
  ...
  //spyglass enable_block SelfDeterminedExpr-ML
  //spyglass enable_block TA_09
          else if (micro_ctrl_i == `ubwc_x2x_MUX_FI_LAST && 
                   aburst_i == `ubwc_x2x_X2X_BT_INCR) begin
  ...
  // spyglass enable_block W116
          ...
          if ( micro_ctrl_i == `ubwc_x2x_MUX_WRAP_BOUND ) mxn_addr = bound_addr_i;

          // jstokes, 7.8.10, crm 9000406128, total_addr_r_i now
          // OPRT_AW+1 bits wide, mxn_addr needs only OPRT_AW bits.
          if ( micro_ctrl_i == `ubwc_x2x_MUX_WRAP ) mxn_addr = total_addr_r_i[OPRT_AW-1:0];

          if ( micro_ctrl_i == `ubwc_x2x_MUX_FI_LAST && 
               aburst_i == `ubwc_x2x_X2X_BT_INCR ) begin
            // jstokes, 7.8.10, crm 9000406128, total_addr_r_i now
            // OPRT_AW+1 bits wide.
        ...
        `ubwc_x2x_MUX_FI_MUL: begin
          if ( aburst_i == `ubwc_x2x_X2X_BT_FIXED )
            mxn_alen = MAX_SP_ALEN_DFLT; // MAX_SP_ALEN - 1;
          else begin //`ubwc_x2x_X2X_BT_INCR
            if ( (`ubwc_x2x_X2X_MAX_SP_ALEN+1) > reduce_num_r_i ) begin
  // spyglass disable_block TA_09
  // SMD: Reports cause of uncontrollability or unobservability and estimates the number of nets whose controllability / observability is impacted.
  // SJ : Since some bits of the both operands are tied to VCC or ground for all configurations, we are getting uncontrollability issue here. But as per the design logic it is valid and we can't force controllability on this net. Hence waiving it.
  ...
  // spyglass enable_block TA_09
          ...
          if ( aburst_i == `ubwc_x2x_X2X_BT_INCR ) begin
            // jstokes, 7.8.10, crm 9000406128, total_addr_r_i now
            // OPRT_AW+1 bits wide.
            if ( total_addr_r_i <= {1'b0, addr_i[OPRT_AW-1:0]} )
              mxn_addr = addr_i[OPRT_AW-1:0];
            ...
            if ( aready_i ) begin
              ...
              if ( (`ubwc_x2x_X2X_MAX_SP_ALEN+1) > reduce_num_r_i )
                mxn_reduce_num = {RDC_NUM_W{1'b0}};
              else
                mxn_reduce_num = reduce_num_r_i - (`ubwc_x2x_X2X_MAX_SP_ALEN+1);
            end
          end
        end
 ...
 //spyglass enable_block W415a
  ...
  always @( reduce_num_r_i or mxi_reduce_num or micro_ctrl_i or 
    ...
    if ( micro_ctrl_i == `ubwc_x2x_MXI_FI )
      mxin_reduce_num_o = mxi_reduce_num;
    else begin
      if ( micro_ctrl_i == `ubwc_x2x_MUX_FI_MUL || micro_ctrl_i == `ubwc_x2x_MUX_FI_LAST )
  ...
  always @( resize_ctrl_i or mxn_alen or mxi_alen or mxui_alen or 
    ...
    case ( resize_ctrl_i )
      `ubwc_x2x_AS_IS_OR_US_INLAST: begin
      ...
      `ubwc_x2x_SINGLE_XACT_INLAST: begin
        if ( aburst_i == `ubwc_x2x_X2X_BT_FIXED )
  //spyglass disable_block TA_09
  //SMD: Reports cause of uncontrollability or unobservability and estimates the number of nets whose controllability / observability is impacted. 
  //SJ : Tool will issue unobservability warning only for those bits which are "not read" or "floating". Since we are not reading those bits we don't need observability. Hence waiving this warning.
  ...
  // spyglass enable_block W116
      ...
      `ubwc_x2x_MUL_XACT_INLAST: begin
        alen_o = mxi_alen;
        total_alen_o = mxi_total_alen;
        total_addr_o = mxi_total_addr;
      end

      `ubwc_x2x_MUL_XACT_NOTLAST: begin
        alen_o = mxn_alen;
