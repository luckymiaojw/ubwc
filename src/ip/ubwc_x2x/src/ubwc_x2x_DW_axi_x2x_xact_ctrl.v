...
module ubwc_x2x_DW_axi_x2x_xact_ctrl (
  //inputs
  resize_ctrl_i,
  aready_i,
  xact_upsize_i,
...
//  parameter MAX_SP_ALEN       = `ubwc_x2x_X2X_MAX_SP_ALEN+1;//{`ubwc_x2x_X2X_SP_BLW{1b1}}+1
  parameter MAX_SP_TOTAL_BYTE = `ubwc_x2x_X2X_MAX_SP_TOTAL_BYTE; //max_asize*
  parameter MAX_MP_TOTAL_BYTE = `ubwc_x2x_X2X_MAX_MP_TOTAL_BYTE; //max_asize*
//  parameter INT_AW            = `ubwc_x2x_X2X_INTERNAL_AW; //Internal address 
                                                  //width
//  parameter SP_BLW            = `ubwc_x2x_X2X_SP_BLW;      //SP burst length width
  parameter MP_BLW            = `ubwc_x2x_X2X_MP_BLW;      //MP burst length width
//  parameter LOG2_SP_SW        = `ubwc_x2x_X2X_LOG2_SP_SW;  //log2^SP_SW
//  parameter MAX_SP_ASIZE      = `ubwc_x2x_X2X_MAX_SP_ASIZE;//bytes -> asize
  parameter LOG2_MP_SW        = (`ubwc_x2x_X2X_LOG2_MP_SW > 0) ?
                                `ubwc_x2x_X2X_LOG2_MP_SW : 1;   //log2^MP_S
...
//  parameter LOG2_BIG_BW       = LOG2_BIG_SW + SP_BLW;//big asize + SP len
  ...
  assign asize_mux= (resize_ctrl_i == `ubwc_x2x_MUL_XACT_NOTLAST) ? `ubwc_x2x_X2X_MAX_SP_ASIZE:
                     mp_asize_mux_i;
  ...
  always @( mp_total_byte_i or aready_i
    ...
    mux_xact_ctrl  = last_rs_xact_ctl_r_i;
      ...
      // SJ : No precision will be lost if we add 2 different width operands.
      if ( aburst_i == `ubwc_x2x_X2X_BT_WRAP ) begin //wrap
        if ( mp_total_byte_i > 
             ((total_alen_r_i + (`ubwc_x2x_X2X_MAX_SP_ALEN+1)) << asize_mux)
           ) 
        begin //over one xact
          // More than 1 SP t/x left (after the current t/x) 
          // to complete the transfer.
          mux_micro_ctrl = `ubwc_x2x_MUX_OVERONE;
 
          if ( aready_i )
            mux_xact_ctrl = 1'b0;
        end 
        else begin //last one or two xacts
          if ( total_addr_r_i == ({1'b0, bound_addr_i} + { {(OPRT_AW-LOG2_MP_BW){1'b0}} , mp_total_byte_i} ) ) begin
            // Next t/x will complete the transfer, current transfer
            // addresses to the upper wrap boundary.
            mux_micro_ctrl = `ubwc_x2x_MUX_WRAP_BOUND;
      // spyglass enable_block SelfDeterminedExpr-ML
          ...
          // OPRT_AW+1 bits wide.
            ...
            mux_micro_ctrl = `ubwc_x2x_MUX_WRAP;

              if ( aready_i ) mux_xact_ctrl = 1'b1;
          end
          else begin //cross boundary
            mux_micro_ctrl = `ubwc_x2x_MUX_CROSS;
     ...
     // SJ : Tool will issue unobservability warning only for those bits which are "not read" or "floating". Since we are not reading those bits we don't need observability. Hence waiving this warnin
        if ( aburst_i == `ubwc_x2x_X2X_BT_FIXED &&
          ...
          mux_micro_ctrl = `ubwc_x2x_MUX_FI_MUL;

          if ( aready_i )
            mux_xact_ctrl = 1'b0;
        end
        else if ( aburst_i == `ubwc_x2x_X2X_BT_INCR &&
          ...
          mux_micro_ctrl = `ubwc_x2x_MUX_FI_LAST;
  ...
  always @( aready_i or 
    ...
    mxi_xact_ctrl  = last_rs_xact_ctl_r_i;
      ...
      if ( aburst_i == `ubwc_x2x_X2X_BT_WRAP ) begin //wrap
        mxi_micro_ctrl = `ubwc_x2x_MXI_WRAP;

        if ( aready_i )
          mxi_xact_ctrl = 1'b0;
      end
      else begin //fixed or incr
        mxi_micro_ctrl = `ubwc_x2x_MXI_FI;
  ...
  wire spbyte_lgr_mp;
  assign spbyte_lgr_mp = (mp_total_byte_i <= `ubwc_x2x_X2X_MAX_SP_TOTAL_BYTE) |
                         SPBYTE_LGR_MP;
      ...
      if ( aburst_i == `ubwc_x2x_X2X_BT_WRAP ) begin //wrap
        ...
        if ( xact_upsize_i ) begin //upsize
          if ( spbyte_lgr_mp &&
            ...
            mxui_micro_ctrl = `ubwc_x2x_MXUI_SINGLE;
  
            if ( aready_i )
              mxui_xact_ctrl = 1'b1;
          end
          else if ( addr_i +  `ubwc_x2x_X2X_MAX_SP_TOTAL_BYTE<=
                    bound_addr_i + mp_total_byte_i) begin //no cross
            mxui_micro_ctrl = `ubwc_x2x_MXUI_NO_CROSS;
   
            if ( aready_i )
              mxui_xact_ctrl = 1'b0;
          end
          else begin //cross, multi xact
            mxui_micro_ctrl = `ubwc_x2x_MXUI_CROSS;
         ...
         //VCS coverage on
          ...
          mxui_micro_ctrl = `ubwc_x2x_MXUI_DS;
      ...
      // spyglass enable_block SelfDeterminedExpr-ML
        ...
        if ( xact_upsize_i ) begin //upsize
          if ( spbyte_lgr_mp ) begin //SingleXact
            mxui_micro_ctrl = `ubwc_x2x_MXUI_FI_SINGLE;

            if ( aready_i )
              mxui_xact_ctrl = 1'b1;
          end
          else begin //multi xact
            mxui_micro_ctrl = `ubwc_x2x_MXUI_FI_MUL;
          ...
          //VCS coverage on 
          mxui_micro_ctrl = `ubwc_x2x_MXUI_FI_DS;
  ...
  always @( resize_ctrl_i or mxui_micro_ctrl or mxui_xact_ctrl or
    ...
    micro_ctrl_o       = `ubwc_x2x_IDLE_CTRL;

    case ( resize_ctrl_i )
      `ubwc_x2x_AS_IS_OR_US_INLAST, `ubwc_x2x_SINGLE_XACT_INLAST: begin
        if ( aready_i )
