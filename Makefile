...
PYTHON       ?= $(shell if command -v python3 >/dev/null 2>&1; then echo python3; elif command -v python3.11 >/dev/null 2>&1; then echo python3.11; else echo python; fi)
...
.PHONY: help bvector comp_enc comp_dec comp_loop run
...
help:
	...
	@echo "  make comp_dec  [CNUM=0001|all]"
	...
	@echo "  make run MODE=dec  CNUM=0001 [FRAME_NUM=1..100] [RAND_OTF=0|1] [RAND_AXI=0|1] [BANK_DLY=1..4] [AXI_READ_DELAY=N]"
...
comp_dec:
	@$(PYTHON) $(FLOW) comp --mode dec --cnum "$(if $(CNUM),$(CNUM),all)"

comp_loop:
	@$(PYTHON) $(FLOW) comp --mode loop --cnum "$(if $(CNUM),$(CNUM),all)"

run:
	@test -n "$(MODE)" || (echo "ERROR: MODE must be enc, dec, or loop"; exit 1)
