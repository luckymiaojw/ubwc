...
module tb_ubwc_dec_tile_to_otf_tajmahal_4096x608_rgba1010102;
    ...
    task automatic send_rgba_tile_from_vector;
        ...
        begin
            ...
            drive_axis_tile_header(FMT_RGBA1010102, tile_x[15:0], tile_y[15:0]);
    ...
    always @(posedge clk_otf or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if ((active_x == 0) && ((active_y % 64) == 0)) begin
                $display("RGBA1010102 vector OTF progress: line %0d / %0d", active_y, IMG_H_STORED);
            end
    ...
    initial begin
        ...
        $readmemh("tajmahal_rgba1010102_tiled.memh",  tiled_words);
        $readmemh("tajmahal_rgba1010102_linear.memh", linear_words);

        if (^tiled_words[0] === 1'bx) begin
            $fatal(1, "Failed to load tajmahal_rgba1010102_tiled.memh");
        end
        if (^linear_words[0] === 1'bx) begin
            $fatal(1, "Failed to load tajmahal_rgba1010102_linear.memh");
        end

        rst_n             = 1'b0;
        cfg_img_width     = IMG_W[15:0];
        cfg_format        = FMT_RGBA1010102;
        ...
        $display("Vector source      : tajmahal_rgba1010102_tiled.memh / tajmahal_rgba1010102_linear.memh");
        ...
        otf_fd = $fopen("tajmahal_rgba1010102_otf_stream.txt", "w");
        if (otf_fd == 0) begin
            $fatal(1, "Failed to open tajmahal_rgba1010102_otf_stream.txt");
        end
        ...
        if (timeout >= 3000000) begin
            $fatal(1, "Timeout waiting for TajMahal RGBA1010102 OTF frame.");
        end

        if (!sending_done) begin
            $fatal(1, "RGBA1010102 AXIS frame was not fully sent.");
        end
        ...
        $display("PASS: ubwc_dec_tile_to_otf TajMahal RGBA1010102 vector completed");
...
`ifdef WAVES
`ifdef FSDB
        $fsdbDumpfile("tb_ubwc_dec_tile_to_otf_tajmahal_4096x608_rgba1010102.fsdb");
        $fsdbDumpvars(0, tb_ubwc_dec_tile_to_otf_tajmahal_4096x608_rgba1010102);
        $fsdbDumpMDA(0, tb_ubwc_dec_tile_to_otf_tajmahal_4096x608_rgba1010102);
`else
        $dumpfile("tb_ubwc_dec_tile_to_otf_tajmahal_4096x608_rgba1010102.vcd");
        $dumpvars(0, tb_ubwc_dec_tile_to_otf_tajmahal_4096x608_rgba1010102);
`endif
`endif
    end
